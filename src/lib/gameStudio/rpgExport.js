...
const ENGINE_JS = [
  ...
  '  function renderStoryHistory(){ var layout=meta.playbackLayout||"timeline"; if(layout==="focus") return ""; var logs=(state.storyLog&&state.storyLog.length)?state.storyLog:state.history.map(function(id){ var n=GAME.nodes[id]||{}; return {nodeId:id,speaker:n.speaker||"",text:n.text||""}; }); var s=\'<div class="rpg-export-history rpg-export-history-\'+layout+\'">\'; for(var i=0;i<logs.length;i++){ var e=logs[i]; s+=\'<article><div><span>Lượt \'+(i+1)+\'</span><b>\'+esc((e.speaker&&String(e.speaker).trim())||"Dẫn truyện")+\'</b></div><p>\'+esc(e.text||"")+\'</p>\'+(e.choiceText?\'<small>Bạn đã chọn · \'+esc(e.choiceText)+\'</small>\':"")+\'</article>\'; } return s+\'</div>\'; }',
  '',
  ...
  '    container.innerHTML=effectLayer()+renderScene(node); bindStatusBar(); bindTabs(); bindDetailToggle();',
