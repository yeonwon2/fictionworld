...
const ENGINE_JS = [
  ...
  '  var litrpg = meta.litrpg || { ranks:["Luyen Khi","Truc Co","Kim Dan","Nguyen Anh"], expPerRank:100 };',
  ...
  '  var LABELS = { rebirth:{t:"Trong Sinh",c:"#a855f7"}, modern:{t:"Tri Thuc Hien Dai",c:"#0ea5e9"} };',
  '  var ENDINGS = { TRUE_END:{l:"Ket Thuc That",i:"V"}, GOOD_END:{l:"Ket Thuc Tot",i:"*"}, NORMAL_END:{l:"Ket Thuc Thuong",i:"="}, BAD_END:{l:"Ket Thuc Xau",i:"X"} };',
  ...
  '',
  '  function showSystemPopup(p){ if(!p||!p.title) return; var c=document.getElementById("rpg-syspopup"); if(!c) return; playTing(); c.innerHTML=\'<div class="rpg-sys-overlay" id="sysOverlay"><div class="rpg-sys-box"><div class="rpg-sys-title"><span class="rpg-sys-icon">✦</span> \'+esc(p.title)+\'</div><div class="rpg-sys-text">\'+esc(p.text||"")+\'</div><button class="rpg-sys-btn" id="sysClose">Da hieu</button></div></div>\'; c.style.display="block"; var ov=document.getElementById("sysOverlay"); var cl=document.getElementById("sysClose"); if(ov) ov.addEventListener("click", function(){ c.innerHTML=""; c.style.display="none"; }); if(cl) cl.addEventListener("click", function(){ c.innerHTML=""; c.style.display="none"; }); }',
  '',
  ...
  '  function showDiceRoll(c){ var c2=document.getElementById("rpg-syspopup"); if(!c2) return; var stat=statLabel(c.diceRoll.stat||""); var bonus=c.diceRoll.stat?Math.floor((state.stats[c.diceRoll.stat]||0)/10):0; c2.innerHTML=\'<div class="rpg-dice-overlay"><div class="rpg-dice-box"><div class="rpg-sys-title">🎲 Quay Xuc Xac Dinh Menh</div><div class="rpg-dice-stat">Dua tren \'+esc(stat)+\' (thuong +\'+bonus+\')</div><div class="rpg-dice" id="diceFace">?</div><div id="diceHint" class="rpg-dice-hint">Dang quay...</div></div></div>\'; c2.style.display="block"; var face=document.getElementById("diceFace"); var hint=document.getElementById("diceHint"); var ticks=0; var iv=setInterval(function(){ ticks++; if(ticks>14){ clearInterval(iv); var r=rollDice(c.diceRoll, state.stats); face.textContent=r.raw; face.className="rpg-dice "+(r.outcome==="success"?"win":"lose"); hint.innerHTML=""; var s1=document.createElement("div"); s1.innerHTML="<b>"+r.raw+(r.bonus>0?" + "+r.bonus:"")+" = "+r.total+" / "+r.diff+"</b>"; hint.appendChild(s1); var s2=document.createElement("div"); s2.className="rpg-dice-res"; s2.style.color=r.outcome==="success"?"#39d14a":"#ff4d4d"; s2.textContent=r.crit==="crit_success"?"CRITICAL THANH CONG!":r.crit==="crit_fail"?"CRITICAL THAT BAI!":(r.outcome==="success"?"THANH CONG":"THAT BAI"); hint.appendChild(s2); var btn=document.createElement("button"); btn.className="rpg-sys-btn"; btn.textContent="Tiep tuc"; btn.addEventListener("click", function(){ c2.innerHTML=""; c2.style.display="none"; resolveDice(c, r); }); hint.appendChild(btn); } else { face.textContent=1+Math.floor(Math.random()*20); face.style.transform="rotate("+(ticks*24)+"deg)"; } },70); }',
  '  function resolveDice(c, r){ var res=applyDice(c.diceRoll, r, state.stats); state.stats=res.stats; pushEvent("🎲", r.crit?(r.outcome==="success"?"CRITICAL thanh cong!":"CRITICAL that bai!"):(r.outcome==="success"?"Quay trung!":"Quay truot...")); var dead=checkGameOver(res.stats); if(!dead){ state.history.push(state.nodeId); if(res.target && GAME.nodes[res.target]) state.nodeId=res.target; } save(); render(); }',
  '',
  '  var combatState=null;',
  '  function startCombat(node){ combatState={ node:node, enemyHp:node.combat.enemy.hp, enemyMaxHp:node.combat.enemy.hp, playerHp:state.stats.hp||100, playerMaxHp:state.stats.hp||100, playerMp:state.stats.mp||state.stats.mana||30, defending:false, turn:"player", over:null, log:[{t:"start",text:(node.combat.enemy.name||"Ke thu")+" xuat hien! "+(node.combat.enemy.intro||"")}] }; renderCombat(); }',
  '  function renderCombat(){ var cs=combatState; if(!cs) return; var node=cs.node; var cb=node.combat; var e=cb.enemy; var s=""; s+=\'<div class="rpg-combat"><div class="rpg-combat-tag">⚔️ Chien Dau</div>\'; s+=\'<div class="rpg-combat-enemy"><img class="rpg-combat-avatar" src="\'+esc(e.avatar||dicebear(e.name))+\'"/><div class="rpg-combat-enemyinfo"><div class="rpg-combat-name">\'+esc(e.name||"Ke thu")+\'</div><div class="rpg-hpbar"><div class="rpg-hpfill" style="width:\'+Math.max(0,Math.min(100,cs.enemyHp/cs.enemyMaxHp*100))+\'%"></div></div><div class="rpg-hpnum">\'+cs.enemyHp+\'/\'+cs.enemyMaxHp+\'</div>\'+(e.intro?\'<div class="rpg-combat-intro">\'+esc(e.intro)+\'</div>\':"")+\'</div></div>\'; s+=\'<div class="rpg-combat-player"><div class="rpg-mpbar-wrap"><span>HP</span><div class="rpg-hpbar"><div class="rpg-hpfill" style="width:\'+Math.max(0,Math.min(100,cs.playerHp/cs.playerMaxHp*100))+\'%"></div></div><span>\'+cs.playerHp+\'</span></div><div class="rpg-mpbar-wrap"><span>MP</span><div class="rpg-mpbar"><div class="rpg-mpfill" style="width:\'+Math.max(0,Math.min(100,cs.playerMp/30*100))+\'%"></div></div><span>\'+cs.playerMp+\'</span></div></div>\'; s+=\'<div class="rpg-combat-log" id="combatLog">\'; for(var i=0;i<cs.log.length;i++){ var l=cs.log[i]; s+=\'<div class="rpg-clog rpg-clog-\'+l.t+\'">\'+esc(l.text)+\'</div>\'; } s+=\'</div>\'; if(!cs.over){ s+=\'<div class="rpg-combat-actions"><button class="rpg-cbtn red" data-act="atk">⚔️ Tan cong</button><button class="rpg-cbtn sky" data-act="def">🛡 Phong thu</button>\'; if(state.skills && state.skills.length){ s+=\'<button class="rpg-cbtn violet" data-act="skill">✨ \'+esc(state.skills[0])+\' (15MP)</button>\'; } s+=\'<button class="rpg-cbtn outline" data-act="flee">🏃 Bo chay</button></div>\'; if(cs.turn==="enemy"){ s+=\'<div class="rpg-clog-hint">Luot dich hanh dong...</div>\'; } } else { s+=\'<div class="rpg-combat-over">\'+(cs.over==="win"?"🏆 Chien thang!":cs.over==="flee"?"🏃 Da rut lui":"💀 That bai...")+\'<button class="rpg-sys-btn" id="combatDone">Tiep tuc</button></div>\'; } s+=\'</div>\'; var container=document.getElementById("game"); container.innerHTML=renderStatusBar()+renderArchBar()+s; bindStatusBar(); var btns=container.querySelectorAll(".rpg-cbtn"); for(var j=0;j<btns.length;j++){ (function(b){ b.addEventListener("click", function(){ combatAct(b.getAttribute("data-act")); }); })(btns[j]); } var cd=container.querySelector("#combatDone"); if(cd) cd.addEventListener("click", finishCombat); }',
  '  function combatAct(act){ var cs=combatState; if(!cs||cs.over||cs.turn!=="player") return; var e=cs.node.combat.enemy; var pAtk=12+(state.rankIndex||0)*3+((state.skills||[]).length*2); if(act==="atk"){ var crit=Math.random()<0.15; var dmg=Math.max(1,Math.round((pAtk-e.defense)*(0.85+Math.random()*0.3))); if(crit) dmg=Math.round(dmg*1.6); cs.enemyHp=Math.max(0,cs.enemyHp-dmg); cs.log.push({t:crit?"crit":"player",text:"Ban tan cong"+(crit?" (CRITICAL!)":"")+" gay "+dmg+" sat thuong."}); if(cs.enemyHp<=0){ cs.over="win"; cs.log.push({t:"win",text:"Ban da danh bai "+(e.name||"ke thu")+"!"}); } else { cs.turn="enemy"; } renderCombat(); if(cs.turn==="enemy"&&!cs.over) setTimeout(combatEnemyTurn,600); return; } if(act==="def"){ cs.defending=true; cs.log.push({t:"defend",text:"Ban gio khien phong thu."}); cs.turn="enemy"; renderCombat(); setTimeout(combatEnemyTurn,600); return; } if(act==="skill"){ if(cs.playerMp<15){ cs.log.push({t:"warn",text:"Khong du MP!"}); renderCombat(); return; } cs.playerMp-=15; var sdmg=Math.max(1,Math.round((pAtk-e.defense)*1.8)+Math.floor(Math.random()*6)); cs.enemyHp=Math.max(0,cs.enemyHp-sdmg); cs.log.push({t:"skill",text:"Ky nang bung no! Gay "+sdmg+" sat thuong."}); if(cs.enemyHp<=0){ cs.over="win"; cs.log.push({t:"win",text:"Chien thang!"}); } else { cs.turn="enemy"; } renderCombat(); if(cs.turn==="enemy"&&!cs.over) setTimeout(combatEnemyTurn,600); return; } if(act==="flee"){ var fc=cs.node.combat.fleeChance!=null?cs.node.combat.fleeChance:0.4; if(Math.random()<fc){ cs.over="flee"; cs.log.push({t:"flee",text:"Bo chay thanh cong!"}); } else { cs.log.push({t:"warn",text:"Bo chay that bai!"}); cs.turn="enemy"; } renderCombat(); if(cs.turn==="enemy"&&!cs.over) setTimeout(combatEnemyTurn,600); return; } }',
  '  function combatEnemyTurn(){ var cs=combatState; if(!cs||cs.over) return; var e=cs.node.combat.enemy; var dmg=Math.max(1,Math.round(e.attack*(0.85+Math.random()*0.3))); if(cs.defending) dmg=Math.max(1,Math.round(dmg*0.4)); cs.defending=false; cs.playerHp=Math.max(0,cs.playerHp-dmg); cs.log.push({t:"enemy",text:(e.name||"Ke thu")+" gay "+dmg+" sat thuong."}); if(cs.playerHp<=0){ cs.over="lose"; cs.log.push({t:"lose",text:"Ban da guc nga..."}); } else { cs.turn="player"; } renderCombat(); }',
  '  function finishCombat(){ var cs=combatState; if(!cs) return; var node=cs.node; var ns=Object.assign({}, state.stats, {hp:cs.playerHp, mp:cs.playerMp}); var inv=state.inventory.slice(); var target=null; if(cs.over==="win"){ var loot=node.combat.loot||{}; if(loot.statModifiers){ for(var k in loot.statModifiers){ ns[k]=(ns[k]||0)+loot.statModifiers[k]; } } if(loot.grantItem && inv.indexOf(loot.grantItem)<0 && inv.length<mysterySlots){ inv.push(loot.grantItem); pushEvent("📦","Nhat duoc: "+loot.grantItem); } if(loot.exp){ var exp=state.exp+loot.exp; var ri=state.rankIndex; var ranks=litrpg.ranks||[]; var per=litrpg.expPerRank||100; while(exp>=per && ri<ranks.length-1){ exp-=per; ri++; pushEvent("⚡","Dot pha! Dat "+ranks[ri]); } ns.exp=exp; ns.rankIndex=ri; } target=node.combat.winTarget; pushEvent("🏆","Chien thang "+(node.combat.enemy.name||"ke thu")+"!"); } else if(cs.over==="flee"){ target=node.combat.fleeTarget; } else { target=node.combat.loseTarget; } state.stats=ns; state.inventory=inv; var dead=cs.over==="lose"||checkGameOver(ns); combatState=null; if(!dead && target && GAME.nodes[target]){ state.history.push(state.nodeId); state.nodeId=target; } save(); render(); }',
  '',
  ...
  '',
  '  function choiceStatus(c){ if(!meetsStatReq(c.statRequirements, state.stats)){ var parts=[]; for(var k in (c.statRequirements||{})){ parts.push(statLabel(k)+" >= "+c.statRequirements[k]+" (can "+(c.statRequirements[k]-(state.stats[k]||0))+")"); } return {ok:false, reason:parts.join(", ")}; } if(c.requiresItem && state.inventory.indexOf(c.requiresItem)<0){ return {ok:false, reason:"Can vat pham: "+c.requiresItem}; } if(c.requiresFlag && state.flags.indexOf(c.requiresFlag)<0){ return {ok:false, reason:"Can co: "+c.requiresFlag}; } return {ok:true}; }',
  '',
  ...
  '    s+=\'<div class="rpg-playerinfo"><div class="rpg-playername">\'+esc(meta.player_name||"Nguoi Choi")+\'</div><div class="rpg-stats">\';',
  ...
  '    s+=\'</div></div></div>\';',
  '    s+=\'<div class="rpg-savebtns"><button id="btnSave">Luu</button><button id="btnLoad">Tai</button><button id="btnReset">Choi lai</button></div>\';',
  '    return s; }',
  '',
  ...
  '    s+=\'<div class="rpg-tabs"><button class="rpg-tab\'+(activeTab==="stats"?" active":"")+\'" data-tab="stats">Chi so</button>\';',
  '    if(showInv){ s+=\'<button class="rpg-tab\'+(activeTab==="inventory"?" active":"")+\'" data-tab="inventory">🎒 Tui do (\'+state.inventory.length+(archetype==="mystery"?"/"+mysterySlots:"")+\')</button>\'; }',
  '    if(showQuest){ var act=0; for(var q in state.quests){ if(state.quests[q].status==="active") act++; } s+=\'<button class="rpg-tab\'+(activeTab==="quests"?" active":"")+\'" data-tab="quests">📜 Nhiem vu (\'+act+\')</button>\'; }',
  ...
  '    if(archetype==="isekai"){ if(state.flags.length){ s+=\'<div class="rpg-tab-sec">🦋 Story Flags</div><div class="rpg-chips">\'; for(var i=0;i<state.flags.length;i++){ s+=\'<span class="rpg-chip">\'+esc(state.flags[i])+\'</span>\'; } s+=\'</div>\'; } var na=state.npcAffinity; var has=false; for(var n in na){ has=true; } if(has){ s+=\'<div class="rpg-tab-sec">💬 Do hao cam NPC</div>\'; for(var n in na){ s+=\'<div class="rpg-tab-row"><span>\'+esc(n)+\'</span><b style="color:\'+(na[n]>=0?"#39d14a":"#ff4d4d")+\'">\'+(na[n]>0?"+":"")+na[n]+\'</b></div>\'; } } }',
  '    if(archetype==="litrpg" && state.skills.length){ s+=\'<div class="rpg-tab-sec">✨ Ky nang</div><div class="rpg-chips">\'; for(var i=0;i<state.skills.length;i++){ s+=\'<span class="rpg-chip">\'+esc(state.skills[i])+\'</span>\'; } s+=\'</div>\'; }',
  '    return s; }',
  '  function renderInventory(){ if(!state.inventory.length) return \'<p class="rpg-empty">Tui do trong.</p>\'; var s=\'<div class="rpg-inv-grid">\'; for(var i=0;i<state.inventory.length;i++){ s+=\'<div class="rpg-inv-slot">🎒 \'+esc(state.inventory[i])+\'</div>\'; } s+=\'</div>\'; return s; }',
  '  function renderQuests(){ var has=false; for(var q in state.quests){ has=true; } if(!has) return \'<p class="rpg-empty">Chua co nhiem vu.</p>\'; var s=""; for(var q in state.quests){ var qq=state.quests[q]; s+=\'<div class="rpg-quest\'+(qq.status==="completed"?" done":"")+\'"><div class="rpg-quest-head"><span>\'+(qq.status==="completed"?"✅":"📜")+\' \'+esc(qq.title)+\'</span><span class="rpg-quest-st">\'+(qq.status==="completed"?"Hoan thanh":"Dang lam")+\'</span></div>\'; if(qq.desc) s+=\'<div class="rpg-quest-desc">\'+esc(qq.desc)+\'</div>\'; if(qq.reward) s+=\'<div class="rpg-quest-rew">🎁 \'+esc(qq.reward)+\'</div>\'; s+=\'</div>\'; } return s; }',
  '',
  ...
  '    s+=\'<div class="rpg-scene"><div class="rpg-scene-head"><div class="rpg-speaker">\'+esc(node.speaker||"")+\'</div><div class="rpg-chapter">Phan canh \'+(state.history.length+1)+\'</div></div>\';',
  '    s+=\'<div class="rpg-dialogue" id="dialogue"></div>\';',
  '    s+=\'<button class="rpg-skip" id="skipBtn">Bo qua</button>\';',
  ...
  '      s+=\'</button>\'; }',
  '    if((node.choices||[]).length===0 && !node.isEnding){ s+=\'<div class="rpg-nochoice">Khong co lua chon. <button id="btnReset2">Choi lai</button></div>\'; }',
  '    el.innerHTML=s;',
  ...
  '    if(c.exp){ exp=state.exp+c.exp; var ranks=litrpg.ranks||[]; var per=litrpg.expPerRank||100; while(exp>=per && rankIndex<ranks.length-1){ exp-=per; rankIndex++; ev.push(["⚡","Dot pha Canh gioi! Dat "+ranks[rankIndex]]); } }',
  '    if(c.systemPoints){ systemPoints=state.systemPoints+c.systemPoints; }',
  '    if(c.grantFlag && flags.indexOf(c.grantFlag)<0){ flags.push(c.grantFlag); ev.push(["🦋","Kich hoat hieu ung Canh buom: "+c.grantFlag]); }',
  '    if(c.grantItem && inv.indexOf(c.grantItem)<0 && inv.length<mysterySlots){ inv.push(c.grantItem); ev.push(["📦","Nhat duoc: "+c.grantItem]); }',
  '    if(c.removeItem){ inv=inv.filter(function(x){ return x!==c.removeItem; }); }',
  '    if(c.unlockSkill && skills.indexOf(c.unlockSkill)<0){ skills.push(c.unlockSkill); ev.push(["✨","Mo khoa ky nang: "+c.unlockSkill]); }',
  '    if(c.completeQuestId && quests[c.completeQuestId]){ quests[c.completeQuestId]=Object.assign({}, quests[c.completeQuestId], {status:"completed"}); ev.push(["✅","Hoan thanh nhiem vu: "+quests[c.completeQuestId].title]); }',
  ...
  '    save(); render(); }',
  '',
  '  function renderSummary(){ var s=\'<div class="rpg-sum-title">Thanh tich</div>\'; for(var i=0;i<statsConfig.length;i++){ var sc=statsConfig[i]; s+=\'<div class="rpg-sum-row"><span>\'+esc(sc.label||sc.key)+\'</span><b>\'+(state.stats[sc.key]||0)+\'</b></div>\'; } s+=\'<div class="rpg-sum-row"><span>So phan canh da qua</span><b>\'+state.history.length+\'</b></div>\'; return s; }',
  '',
  '  function renderGameOver(){ return \'<div class="rpg-bg"></div><div class="rpg-ending"><div class="rpg-ending-badge bad">GAME OVER</div><div class="rpg-ending-text">Ban da guc nga. So phan da khep lai qua som...</div><div class="rpg-summary">\'+renderSummary()+\'</div><button class="rpg-restart" id="goRestart">Bat dau lai</button></div>\'; }',
  '',
  '  function renderEnding(node){ var et=node.endingType||"NORMAL_END"; var em=ENDINGS[et]||{l:"Ket Thuc",i:"T"}; return \'<div class="rpg-bg" style="background-image:url(\\\'\'+esc(node.bgImage||"")+\'\\\')"></div><div class="rpg-ending"><div class="rpg-ending-badge \'+et.toLowerCase()+\'">\'+em.i+" "+em.l+\'</div><div class="rpg-ending-text">\'+esc(node.text||"")+\'</div><div class="rpg-summary">\'+renderSummary()+\'</div><button class="rpg-restart" id="endRestart">Choi lai tu dau</button></div>\'; }',
  '',
  ...
  '',
  '  function render(){ clearTyping(); var node=GAME.nodes[state.nodeId]; if(!node){ state.nodeId="start_node"; node=GAME.nodes["start_node"]; } if(!node){ document.getElementById("game").innerHTML=\'<div class="rpg-error">Khong co du lieu game.</div>\'; return; }',
  ...
  '    if(node.randomEvents && node.randomEvents.length){ for(var ei=0;ei<node.randomEvents.length;ei++){ var ev=node.randomEvents[ei]; if(Math.random()<(ev.chance||0.2)){ if(ev.statModifiers){ for(var ek in ev.statModifiers){ ns[ek]=(ns[ek]||0)+ev.statModifiers[ek]; } } if(ev.grantItem && inv.indexOf(ev.grantItem)<0 && inv.length<mysterySlots){ inv.push(ev.grantItem); } pushEvent(ev.icon||"🎲", ev.text||"Su kien bat ngo!"); } } }',
  '    if(node.systemPopup && node.systemPopup.title){ showSystemPopup(node.systemPopup); }',
  '    if(node.grantItem && inv.indexOf(node.grantItem)<0 && inv.length<mysterySlots){ inv.push(node.grantItem); pushEvent("📦","Nhat duoc: "+node.grantItem); }',
  '    if(node.setFlags){ for(var i=0;i<node.setFlags.length;i++){ if(flags.indexOf(node.setFlags[i])<0){ flags.push(node.setFlags[i]); pushEvent("🦋","Kich hoat hieu ung Canh buom: "+node.setFlags[i]); } } }',
  '    if(node.quest && node.quest.id && !quests[node.quest.id]){ quests[node.quest.id]=Object.assign({}, node.quest, {status:"active"}); pushEvent("📜","Nhiem vu moi: "+node.quest.title); }',
