...
export const POPUP_KINDS = {
  system: 'Hệ thống', edict: 'Thánh chỉ', decree: 'Chiếu thư', quest: 'Nhiệm vụ', opportunity: 'Cơ hội', custom: 'Thông báo',
};
...
export function applyWriting(game, keys, result) {
  ...
  for (const entry of result.entries) {
    ...
    if (scope.choiceIndex === null) {
      ...
      const popupKind=node.workshopPopupKind||(game.meta.aiWorkshop?.type==='system'?'system':'');
      if(popupKind){
        if(node.workshopPopupKind&&!entry.systemText?.trim())throw new Error(`${scope.id}: cảnh đã đánh dấu ${POPUP_KINDS[popupKind]||'thông báo'} nhưng AI chưa viết nội dung popup.`);
        if(entry.systemText)node.systemPopup={title:entry.systemTitle||node.systemPopup?.title||POPUP_KINDS[popupKind]||'Thông báo',text:requiredText(entry.systemText,'thông báo nổi')};
      }
    }
...
export function workshopPrompt(game, keys = null, instruction = '') {
  ...
  const popupTargets=selectedScopes(game,keys).filter(scope=>scope.choiceIndex===null&&game.nodes[scope.id].workshopPopupKind).map(scope=>({key:scope.key,kind:game.nodes[scope.id].workshopPopupKind,label:POPUP_KINDS[game.nodes[scope.id].workshopPopupKind]||game.nodes[scope.id].systemPopup?.title||'Thông báo'}));
  return base + modeRule + `Vai trò cảnh hệ quả và quyết định dẫn vào (đọc kỹ): ${JSON.stringify(consequences)}. Nút workshopContinuation phải giữ nguyên điểm hiện tại; không tính lại hệ quả cơ chế.\nTHÔNG BÁO NỔI ĐÃ ĐƯỢC TÁC GIẢ ĐÁNH DẤU: ${JSON.stringify(popupTargets)}. Với đúng các cảnh này, systemTitle/systemText là nội dung bảng popup hiện riêng trên màn hình, KHÔNG phải lời thoại của nhân vật. Thánh chỉ/chiếu thư dùng văn phong văn bản ban xuống; hệ thống/nhiệm vụ dùng thông báo giao diện ngắn gọn. Không nhân cách hóa "Hệ thống", không cho popup nói chuyện qua lại, không chép nguyên popup vào text của cảnh. Cảnh không được đánh dấu thì để systemTitle/systemText rỗng.\nChỉ viết các phạm vi sau: ${JSON.stringify(selectedScopes(game, keys))}. key phải khớp. choiceIndex=null: viết toàn cảnh và tất cả lựa chọn; choiceIndex là số: chỉ viết lựa chọn đó, text/speaker của entry để rỗng. Mỗi lựa chọn có index, text, modifiers gồm key/value từ statsConfig. Không tạo chỉ số mới. Với lựa chọn đã có npcCard, điền npcName và npcTagline theo nhân vật đã thống nhất trong bible; không tự tạo thẻ ở các lựa chọn khác.\nVĂN PHONG CẢNH TRUYỆN: Viết như một phân đoạn phim hoặc chương truyện tương tác, không viết tóm tắt sự kiện. Cảnh thường cần có (1) mở cảnh rõ không gian, thời điểm và trạng thái nhân vật; (2) hành động, đối thoại, biểu cảm hoặc chi tiết giác quan cụ thể; (3) xung đột hay thông tin làm tình thế thay đổi; (4) nhịp kết tự nhiên đặt người chơi trước lựa chọn. Không dùng danh sách, tiêu đề con, lời giải thích cho tác giả, câu chung chung kiểu “mọi chuyện tiếp diễn”, hoặc kể lướt nhiều sự kiện để cho đủ chữ. Mỗi cảnh phải nối mượt từ cảnh trước và tạo đà cho cảnh đích. Lựa chọn phải là hành động/đối thoại tự nhiên mà nhân vật thật sự có thể làm, không ghi lộ “đáp án tốt/xấu/chết”. Cảnh hệ quả tập trung mô tả ngay phản ứng và cái giá của quyết định vừa chọn, có chuyển tiếp tới cảnh sau, không kể lại quyết định. Kết thúc phải nêu hậu quả cụ thể và dư âm cảm xúc.\nViết nội dung hoàn chỉnh, không chỉ dàn ý. Tôn trọng workshopHint của mỗi cảnh. Cảnh workshopRole=main là cảnh chính mới, không kế thừa vai trò hệ quả của cảnh dẫn vào. Cảnh workshopRole=side mở rộng câu chuyện theo yêu cầu tác giả, không mặc định là hệ quả. Cảnh workshopRole=consequence phải viết hệ quả của đáp án đi vào nó; không tính lại điểm/vật phẩm/cờ đã áp dụng ở đáp án nguồn. Nút Tiếp tục chỉ chuyển cảnh, không tự thêm phần thưởng hoặc chi phí. Các điều kiện statRequirements/statRequirementsMax, cờ/vật phẩm trên đáp án là luật thực thi: viết lựa chọn và HE/BE phù hợp các điều kiện đó. Không tự hứa một kết cục không được luật bảo đảm. Không xuất targetNodeId hoặc trường cấu trúc. Trả entries và suggestions.`;
}
