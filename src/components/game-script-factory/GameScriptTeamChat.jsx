...
import { buildGameScriptBlock, buildGameTeamChatPrompt, GAME_TEAM_ROLES } from "@/lib/gameScriptFactory/prompts";

const HISTORY_CAP = 40;

// Team AI theo vai cho xưởng kịch bản game — mỗi thành viên có hội thoại riêng,
// cùng đọc bộ tài liệu kịch bản + nguyên tắc loại game làm ngữ cảnh.
export default function GameScriptTeamChat({ currentStoryId, gameType, docsByKey }) {
  const [roleKey, setRoleKey] = useState(GAME_TEAM_ROLES[0].key);
  ...
  const role = GAME_TEAM_ROLES.find((r) => r.key === roleKey) || GAME_TEAM_ROLES[0];
  const storageKey = `fw_game_script_chat_${currentStoryId}_${roleKey}`;
  ...
  const docsText = useMemo(() => buildGameScriptBlock(docsByKey), [docsByKey]);
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const prompt = buildGameTeamChatPrompt({ role, gameType, docsText, history: withUser, question });
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden flex flex-col h-[calc(100vh-240px)] min-h-[480px]">
      <div className="px-4 py-2.5 border-b border-border bg-muted/30">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-semibold text-sm">Team AI · Xưởng Kịch Bản Game</span>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {messages.length === 0 && (
          <p className="text-xs text-muted-foreground">
            Hỏi thành viên này bất cứ điều gì — họ luôn đọc full bộ tài liệu kịch bản + nguyên tắc loại game hiện tại.
          </p>
        )}
        {messages.map((m, i) => (
