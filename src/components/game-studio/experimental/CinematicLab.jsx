...
export default function CinematicLab({onBack}) {
 ...
 return <GenreLab key={genreId} genre={genreDemos[genreId]} onGenre={setGenreId} onBack={onBack}/>;
}
function GenreLab({ onBack,genre,onGenre }) {
  ...
  const panelTitle = {system:'Thông báo hệ thống',menu:'Menu game',journal:'Nhật ký hành trình',settings:'Cài đặt đọc',restart:'Chơi lại từ đầu?',exit:'Thoát game?'}[panel];
  const choose=choice=>{
    if(ended)return;
  ...
  return <div className={`cinema-lab cl-genre-${genre.id}`}>
    ...
    <div className="cl-samples"><label>Xem trước kết thúc<select value={endingPreview??''} onChange={e=>{setEndingPreview(e.target.value===''?null:Number(e.target.value));setNotice('');}}><option value="">Theo lượt đang chơi</option>{endingOptions.map((e,i)=><option key={i} value={i}>{e.label} · {e.title}</option>)}</select></label></div>
    ...
    <div className={`cl-stage ${device==='mobile'?'cl-mobile':''}`}>
      <section ref={gameFrame} className={`cl-game ${large?'cl-large':''}`} aria-label={`Bản chơi mẫu ${genre.label}`}>
        <div className="cl-art" style={{backgroundImage:`url(${genre.art})`}} aria-hidden="true"/><div className="cl-shade" aria-hidden="true"/>
        <header ref={headerFrame} className="cl-top" style={{visibility:fixedHeader?'hidden':undefined}}>{headerContents}</header>
        {fixedHeader&&createPortal(<div className={`cinema-lab cl-fixed-chrome cl-genre-${genre.id}`} style={{...fixedHeader,position:'fixed',padding:0,zIndex:35}}><header className="cl-top">{headerContents}</header></div>,document.body)}
