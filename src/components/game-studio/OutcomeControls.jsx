...
import ScoreEndingTemplate from './ScoreEndingTemplate';
...
export default function OutcomeControls({game,onChange,onFocus,initiallyOpen=false}) {
  ...
  return <details open={initiallyOpen||undefined} className="studio-panel p-4"><summary className="cursor-pointer font-semibold">Điểm số, HE/BE và điều kiện kết thúc · {endings.length} kết thúc</summary><div className="space-y-3 mt-3"><p className="text-sm">{game.meta.outcomeMode==='accumulation'?'Chỉ tích lũy điểm; không tự thua vì điểm thấp.':game.meta.outcomeMode==='survival'?'Đang cho phép ngưỡng thua sớm.':'Chưa chốt chế độ điểm; đang dùng cấu hình chỉ số hiện có.'} HE/BE phụ thuộc các đường đi bạn nối, không chỉ nhãn của ô kết thúc.</p><div className="studio-toolbar"><Button variant="outline" onClick={()=>setModeOpen(true)}>Chế độ điểm & lời báo thua</Button><Button variant="outline" onClick={()=>setEnding('new')}>Thêm kết thúc HE/BE/NE</Button><Button onClick={()=>setTemplateOpen(true)}>Mẫu xét kết thúc theo điểm · Chạy thử</Button></div>
    ...
    {templateOpen&&<ScoreEndingTemplate game={game} onClose={()=>setTemplateOpen(false)} onApply={({game:next,checkpoint})=>{onChange(next);onFocus(checkpoint);}}/>}
