...
export default function AIMapWorkshop({ gameData, setGameData, onGenerated, requestAI = aiCall }) {
  ...
  const [maxCalls, setMaxCalls] = useState(3), [targetChars, setTargetChars] = useState(1500), [proseOnly, setProseOnly] = useState(true);
  ...
  return <fieldset disabled={!!busy} className="space-y-4 min-w-0">
    ...
    {workspace && <>
      <details ref={contextPanel} tabIndex={-1} className="studio-panel">
        ...
        <p className="text-xs text-muted-foreground">Dùng Cài đặt AI hiện có. Mỗi lần viết gửi sơ đồ, bối cảnh và nội dung đã duyệt; hệ thống tự gom nhiều ô nhất có thể theo độ dài đầu ra. Ngân sách 1–5 lượt bên dưới áp dụng cho viết ô, không bao gồm nút đề xuất bối cảnh riêng. AI không tự đổi đường nối.</p>
      ...
      <details open ref={writingPanel} className="studio-panel"><summary>Viết lại với AI · {scopes.length} ô đã chọn · Tối đa {maxCalls} lượt</summary><div className="studio-panel-body space-y-3">
        ...
        <div className="studio-writing-options text-sm">
          <label>Ngân sách viết <select aria-label="Ngân sách viết AI" className={selectClass} value={maxCalls} onChange={e=>setMaxCalls(Number(e.target.value))}><option value={1}>Tối đa 1 lượt</option><option value={2}>Tối đa 2 lượt</option><option value={3}>Tối đa 3 lượt · mặc định</option><option value={4}>Tối đa 4 lượt</option><option value={5}>Tối đa 5 lượt</option></select></label>
        ...
        <p className="text-xs text-muted-foreground">Ước tính {writingEstimate.estimatedCalls} lượt cho {scopes.length} ô; lượt bấm này tối đa {maxCalls} cuộc gọi, kể cả sửa phản hồi lỗi. {writingEstimate.remainingKeys.length?`Dự kiến xử lý ${writingEstimate.plannedKeys.length} ô trước, ${writingEstimate.remainingKeys.length} ô để lần tiếp theo.`:'Dự kiến đủ ngân sách; không bảo đảm AI trả đủ nếu nội dung dài hoặc lỗi.'} Một lượt thường gom khoảng 8 cảnh loại 1.500 ký tự hoặc 5 cảnh loại 3.000 ký tự; số thực tế giảm nếu cảnh có nhiều đáp án. Cảnh quá ngắn được coi là chưa hoàn tất và có thể dùng một lượt để sửa.</p>
