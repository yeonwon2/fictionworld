...
export default function ScoreEndingTemplate({game,onApply,onClose}) {
 ...
 {valid&&<><div className="rounded-lg border p-3 text-sm space-y-1"><strong>Các hệ quả cuối truyện → Tự xét điểm (ẩn) → {ranges.length} nhánh</strong>{ranges.map(r=><p key={r.id}>↳ {label} {rangeText(r)} → {r.title}</p>)}</div>
 ...
 <p className="text-sm">Tạo riêng 1 ô xét điểm ẩn (người chơi chuyển thẳng ending, không bấm thêm) và {rules.length} ô kết thúc, không tự nối vào truyện. Sau đó nối hệ quả cuối vào cảnh xét điểm. Lời kết mẫu có thể sửa hoặc để AI viết lại.</p>
