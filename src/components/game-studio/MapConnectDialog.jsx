...
export default function MapConnectDialog({ gameData, request, onApply, onClose }) {
  ...
  const [selected, setSelected] = useState(() => ports.filter((p) => choiceIndex !== null || !p.target || !gameData.nodes[p.target]).map((p) => p.key));
  const [count, setCount] = useState(4), [ending, setEnding] = useState(false);
  ...
  function apply() {
    try {
      ...
      const result = connectFromCard(gameData, { sourceId, choiceIndex, create, portKeys: selected, targetId: target, choiceCount: ending ? 0 : count, ending, hint });
