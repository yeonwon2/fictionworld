...
export default function MindMapSystemPopup({ node, choiceIndex, onSave }) {
  ...
  return <>
    <div className="mt-2 rounded-lg border border-cyan-500/50 bg-cyan-500/10 p-2 whitespace-normal" aria-label="Thông báo hệ thống">
      <strong className="block text-xs">✦ {popup?.title || 'Thông báo hệ thống'}</strong>
    ...
    {draft && <Dialog open onOpenChange={(open) => !open && setDraft(null)}><DialogContent className="max-w-xl" onClick={(e) => e.stopPropagation()}><DialogHeader><DialogTitle>Sửa thông báo hệ thống</DialogTitle><DialogDescription>{choice ? 'Thông báo gắn với lựa chọn này.' : 'Thông báo gắn với cảnh này.'} Không thêm cảnh hoặc thay đường nối.</DialogDescription></DialogHeader>
