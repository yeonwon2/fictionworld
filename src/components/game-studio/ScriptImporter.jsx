...
import { Bot, FileText, Wand2, Loader2, AlertTriangle, ListPlus, GitBranchPlus, FlagTriangleRight, ClipboardCheck, CheckCircle2, Copy, ClipboardList, Lightbulb, Trash2, EyeOff } from "lucide-react";
...
export default function ScriptImporter({ gameData, setGameData, onGenerated }) {
  ...
  // Xoá thẳng dòng "→ Cờ: X"/"→ Vật phẩm: X" khỏi kịch bản từ gợi ý mồ côi —
  // không cần tự tìm dòng đó nằm ở đâu. An toàn vì gợi ý này nghĩa là KHÔNG
  // lựa chọn nào đang cần tới nó, nên xoá không đổi cách chơi.
  const handleDeleteOrphan = (w) => {
    ...
    try {
      setWarnings(parseScript(nextScript, gameData.meta).warnings || []);
      setChecked(true);
    } catch { /* lỗi cú pháp nghiêm trọng — để nút Kiểm Tra báo lại */ }
    toast({ title: `Đã xoá ${removedCount} dòng cấp "${parsed.name}"`, description: parsed.kind === "item" ? "Đã bỏ khỏi kịch bản." : "Đã bỏ khỏi kịch bản." });
  };

  // "Giữ" chỉ ẩn gợi ý này khỏi danh sách đang xem (không lưu lại) — lần
  // "Kiểm Tra" sau sẽ hiện lại nếu cờ/vật phẩm vẫn thật sự mồ côi.
  ...
  const handleAIFix = async () => {
    ...
    let parseWarnings;
    try {
      parseWarnings = parseScript(script, gameData.meta).warnings || [];
    } catch (e) {
    ...
    try {
      const report = await verifyAndFixScript({
        ...
        parseFn: (txt) => parseScript(txt, gameData.meta),
        script,
        maxRounds: 3,
        focusWarnings,
      });
  ...
  const handleAIWriteVerified = async () => {
    ...
    setAiLoading(true);
    try {
      const draft = await generateScriptFromPrompt({ mode: aiMode, input: aiInput, length: aiLength });
      setScript(draft);
      const report = await verifyAndFixScript({
        cheatSheet: CHEAT_SHEET,
        parseFn: (txt) => parseScript(txt, gameData.meta),
