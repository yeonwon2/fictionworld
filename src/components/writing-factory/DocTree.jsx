...
import { Folder, FileText, Sparkles, Loader2, BookText, FolderOpen, Check } from "lucide-react";
import { DOC_DEFS } from "@/lib/writingFactory/prompts";
...
// Cây thư mục của Xưởng Viết Truyện — mô phỏng cấu trúc thư mục tác giả:
// Xưởng/ <tên truyện>/ {00_QUY_TAC_VIET.md ... chapters/ summaries/}
export default function DocTree({ docsByKey, activeKey, onSelect, onBootstrap, bootstrapping, hasContent }) {
  return (
    <div className="rounded-2xl border border-border bg-card overflow-hidden flex flex-col">
      <div className="px-4 py-3 border-b border-border bg-muted/30">
        <div className="flex items-center gap-2">
          <Folder className="w-4 h-4 text-primary" />
          <span className="font-display font-semibold text-sm">Xưởng Viết Truyện</span>
        </div>
        <p className="text-[11px] text-muted-foreground mt-0.5">
          Bộ tài liệu sống (bible) — nơi AI "nhớ" mọi thứ để viết chương không lệch.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        <div className="px-2.5 py-2 rounded-lg text-xs text-muted-foreground bg-muted/40">
          <div className="flex items-center gap-1.5">
            <FolderOpen className="w-3.5 h-3.5" /> chapters/ · summaries/
          </div>
          <p className="text-[10px] mt-1">Chương do Xưởng viết sẽ lưu vào bảng chapters; tóm tắt hiện tại nằm trong tài liệu Tóm Tắt Hiện Tại.</p>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-border">
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5">
          AI dựng 8 tài liệu từ ý tưởng + dữ liệu sổ tay thế giới hiện có.
        </p>
      </div>
    </div>
  );
