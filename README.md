...
**Cân bằng điểm tiết kiệm:** mặc định tối đa 1 cuộc gọi mỗi lần bấm, có thể chọn 2; mỗi nhóm tối đa 40 kết quả đáp án. Giao diện hiển thị dự kiến số lượt và số yêu cầu thực tế, không tự thử lại JSON ngoài ngân sách. Ngữ cảnh giữ luật toàn sơ đồ nhưng chỉ gửi lời truyện của nhóm hiện tại và cảnh đích liền kề; không gửi ảnh, bản sao kịch bản hay vị trí ô. Phản hồi giống hệt được cache. Kết quả đã kiểm tra (kể cả giữ nguyên điểm) được giữ trong cửa sổ để “Tiếp tục” chỉ xử lý phần thiếu; đóng cửa sổ mất phiên duyệt. Ít lượt không bảo đảm ít token hoặc đủ kết quả. Chỉ các dòng được tác giả duyệt mới thay đổi điểm.
...
Bản HTML độc lập và iframe cũng thực hiện xét kết thúc tự động sau hiệu ứng của đáp án cuối, dùng cùng bộ kiểm tra cấu trúc với player trong xưởng. Ô xét điểm không hiện lời truyện/lựa chọn, không tính vào số cảnh hoặc nhật ký hiển thị. Không có hoặc có nhiều kết thúc hợp lệ thì hiện lỗi cấu hình thay vì yêu cầu người chơi chọn. File HTML đã tải trước bản sửa cần được xuất và đăng lại.
...
Khi viết nhiều ô bằng AI, xưởng kiểm tra từng entry theo đúng key và số lựa chọn. Ô ngoài yêu cầu bị bỏ qua; ô thiếu/trùng/sai cấu trúc được yêu cầu bổ sung riêng tối đa một lượt trong nhóm. Các ô hợp lệ được đưa vào ngữ cảnh lượt bổ sung và không bị viết lại. Nếu vẫn thiếu hoặc lỗi mạng/hạn mức, dừng các nhóm sau và mở bản duyệt cho phần đã viết hợp lệ. Sau khi áp dụng, những ô chưa hoàn tất được chọn sẵn để tiếp tục. Không tự áp nội dung vào game; bản đề xuất chỉ nằm trong phiên đang mở cho tới khi tác giả duyệt. Phản hồi của lượt lỗi cũ trước thay đổi này không thể phục hồi từ giao diện.

### Kéo nối và dựng sơ đồ theo yêu cầu

- Kéo **tiêu đề ô** để đổi vị trí. Vị trí cảnh/đáp án được lưu trong `workshopPosition` cùng game; nút **Sắp xếp lại vị trí** bỏ các tọa độ này để bố trí tự động. Sao chép nhóm không giữ tọa độ cũ để tránh chồng bản sao.
- Kéo **chấm tím bên phải đáp án** vào bất kỳ cảnh trước/sau hoặc ending để đổi đích thật, giữ nguyên điểm và điều kiện. Xúc xắc/chiến đấu có chấm riêng theo kết quả (xem chú thích khi rê chuột). Cảnh chưa có đáp án có chấm **Đi tiếp**, tự thêm Bắt đầu/Tiếp tục khi nối.
- **Nhấp đúp/chuột phải trên nền**, hoặc thả dây vào nền: tạo đúng một cảnh tại vị trí đó, chọn cảnh chính/phụ/kết thúc và số đáp án. Thả dây còn nối nguồn vào cảnh mới. Các thao tác kéo bị tắt khi xem từng tuyến.
- **Dựng sơ đồ / AI thiết kế theo yêu cầu** có khung hội tụ hoặc các tuyến song song. Chọn số cảnh chính, số đáp án và hệ quả riêng mỗi đáp án. Tối đa 300 cảnh mới mỗi lần (tính cả hệ quả), không giới hạn cứng ba ending. Xem trước rồi áp dụng; các đích cuối chưa nối để tác giả chọn ending sau.
- Trong cùng cửa sổ, **AI thiết kế theo yêu cầu** dùng cấu hình AI hiện có, gửi ngữ cảnh game để đề xuất ô mới, nội dung kết thúc, điều kiện, chỉ số/ngưỡng thua và đường nối từ cảnh cũ. Bản đề xuất có sơ đồ xem trước, nội dung/điều kiện từng ô và danh sách đường nguồn sẽ đổi. Không xóa cảnh cũ, không tự áp dụng, từ chối nếu dữ liệu đã thay đổi trong lúc AI viết.
- Luật dùng đúng engine hiện có: cổng điểm min/max kết hợp nhiều chỉ số (AND), cờ/vật phẩm, thua khi chỉ số sinh tồn ≤ ngưỡng và ô xét ending ẩn. OR biểu diễn bằng nhiều đường điều kiện rời nhau cùng dẫn một ending; vẫn phải đúng một đường hợp lệ, kể cả khi cùng đích. Luật chỉ xét ở cuối truyện dùng router tại mốc đó và chỉ số không sinh tồn (`isVital=false`), không gây thua ngay từ điểm khởi đầu. Ví dụ ba chỉ số và 4 TRUE_END + 1 BAD_END không vượt khả năng engine; 30 cảnh chính + 120 hệ quả + router + 5 ending là 156 ô, trong giới hạn 300. AI phải báo luật thực sự chưa biểu diễn được (ví dụ thua toàn cục ở ngưỡng trên, bộ đếm thời gian thực hoặc so sánh hai chỉ số động); bản có cơ chế chưa hỗ trợ không được áp dụng một phần. Thông báo này được ghi rõ là nhận định của AI, không phải kết luận kiểm tra engine. Sau khi cập nhật, cần yêu cầu AI tạo lại bản thiết kế đã bị từ chối; bản cũ không tự sửa. Vẫn chạy QA và chơi thử sau khi duyệt để kiểm tra độ phủ điều kiện, khả năng đạt điểm và logic câu chuyện.

### Nhập kịch bản và viết lại tiết kiệm lượt AI

Trong **Dựng sơ đồ / AI thiết kế theo yêu cầu → Nhập kịch bản · Tiết kiệm AI**, dán văn bản hoặc mở `.txt`/`.md`, rồi chọn **Kiểm tra & dựng trên máy · 0 lượt AI**. Bộ đọc giữ nội dung cảnh, lời dẫn, đáp án nhiều dòng và lời kết; nhận tiêu đề `CẢNH`/`PHÂN CẢNH`/`SCENE`, mũi tên `→`/`->`/`=>`, chỉ số khởi đầu, delta, min/max, cờ/vật phẩm và `KẾT THÚC mã [TRUE_END]` (hoặc GOOD_END/NORMAL_END/BAD_END). Các dòng thông báo hệ thống trong lời kể được giữ dưới dạng văn bản, không chuyển thành popup riêng. Phần chú thích/tiêu đề giai đoạn hiện trong ghi chú nhập để duyệt.

`→ Kết thúc theo chỉ số` nối vào router ẩn dùng điều kiện số của các ending. Máy kiểm tra điều kiện thiếu/chồng, đích thiếu, mã trùng, cảnh mồ côi và không nối ngầm theo thứ tự. Điều kiện nghiêm `<`/`>` chỉ quy đổi trên điểm nguyên. Luật sinh tử cuối sự kiện phải có mốc và BAD_END tương ứng; không áp thành ngưỡng sinh tồn ngay đầu game. Không cam kết hiểu mọi định dạng, OR tự do hoặc cơ chế chưa có: phần chưa hiểu được báo theo dòng và chặn áp dụng, không bỏ qua luật.

Khi có dòng luật cần chuẩn hóa, **Nhờ AI làm rõ riêng các dòng luật** chỉ gửi danh sách cảnh/điểm và dòng lỗi, không gửi toàn bộ lời truyện. Chọn tối đa 1 hoặc 2 lượt, bao gồm thử lại JSON; xem trước từng dòng sửa và câu hỏi cần xác nhận. AI không được chèn lời truyện, cảnh mới hoặc sửa dòng ngoài phạm vi. Các vấn đề thiếu ý định/bố cục vẫn cần tác giả sửa. Áp dụng chỉ thêm các ô mới; chỉ tự nối từ ô chưa có đáp án, không ghi đè nhánh cũ. Bộ điểm được cập nhật toàn game nên cần duyệt ảnh hưởng lên các cảnh đã có. Không sửa database/schema.

Trong **Sơ Đồ AI**, chọn nhiều ô → đặt **Ngân sách viết AI** (1 hoặc 2 lượt), **Độ dài viết lại** và **Chỉ trau chuốt lời văn** (mặc định bật). Nhóm được ước tính theo độ dài và số đáp án thay vì cố định 4 ô. App dự báo số lượt/ô có thể xử lý; đây là ước tính, không bảo đảm tất cả ô dài sẽ xong trong 2 lượt. Nếu AI thiếu ô, chỉ hỏi bổ sung phần còn thiếu trong ngân sách; không có lượt gọi JSON ngầm. Lỗi mạng/quota dừng lượt. Kết quả hợp lệ được giữ trong bản đề xuất; duyệt xong, các ô còn thiếu được chọn để bạn chủ động viết tiếp bằng ngân sách mới. Đóng/tải lại trước khi duyệt sẽ bỏ bản đề xuất.

Chế độ chỉ trau chuốt khóa bằng code toàn bộ điểm, điều kiện, đường nối, cờ, vật phẩm, nhân vật, popup và cơ chế khác; chỉ thay `node.text` và `choice.text`. Không cần gọi AI dựng bối cảnh trước để sửa lời văn đã có. Tắt tùy chọn để dùng luồng viết kèm đề xuất điểm (cần duyệt bối cảnh/bộ điểm). Chọn ô cảnh bao gồm các đáp án; chọn riêng đáp án không viết lại cả cảnh; ô xét kết thúc ẩn không được viết lại. Nút AI thiết kế sơ đồ cũng có ngân sách 1/2 lượt. Các nút AI khác (đề xuất bối cảnh, QA, cân bằng điểm) là lượt riêng. Phí token tùy cấu hình nhà cung cấp; ít lượt không tự động đồng nghĩa ít tiền. Ngữ cảnh viết vẫn có giới hạn 240.000 ký tự và không bị cắt âm thầm.

### Duyệt cân bằng điểm bằng AI

Trong **Sơ Đồ Tư Duy**, nút **Cân bằng điểm bằng AI** nằm cạnh công cụ dựng sơ đồ. Chọn một chỉ số, toàn kịch bản/một cảnh/các ô đã chọn, nhập yêu cầu rồi lấy đề xuất. Mỗi dòng có đáp án, điểm delta cũ/mới và lý do; mặc định không dòng nào được duyệt. **Áp dụng N dòng đã duyệt** chỉ thay delta của chỉ số đó trên những đáp án đã tích, không đổi lời truyện, liên kết, điều kiện, điểm ban đầu hay ngưỡng ending.

AI dùng cấu hình hiện có và ngữ cảnh game đầy đủ, tối đa 20 kết quả đáp án mỗi lượt; giới hạn 240.000 ký tự ngữ cảnh, không tự cắt truyện. Nhóm lỗi không áp dụng gì, các nhóm đã hoàn thành vẫn có thể được duyệt. Các kết quả xúc xắc được phân biệt thành công/thất bại; phần thưởng chiến đấu/sự kiện, ô xét ending và nút Tiếp tục hệ quả không thuộc phạm vi này. Bản đề xuất hết hiệu lực khi game thay đổi. Sau khi duyệt một phần hay toàn bộ, chạy QA và chơi thử để xác minh khả năng đạt điểm/kết thúc; đề xuất AI không thay thế kiểm tra đó.

Cân bằng điểm giữ cả kết quả “giữ nguyên” để đếm đúng tiến độ: số kết quả đáp án đã kiểm tra, số cảnh đã hoàn tất, số thay đổi và số còn thiếu. Nếu AI bỏ sót/sai dòng, xưởng giữ dòng hợp lệ và tự hỏi lại đúng phần thiếu với nhóm nhỏ hơn (20 → 5 → 1, có giới hạn). Khi API lỗi hoặc vẫn thiếu, dùng **Tiếp tục N dòng còn thiếu** ngay trong cửa sổ để tiếp tục mà không gọi lại phần đã kiểm tra hoặc mất các dấu duyệt. Đóng cửa sổ/tải lại trang sẽ bỏ phiên đề xuất chưa áp dụng; thay đổi game làm phiên hết hiệu lực. Bảng chỉ liệt kê đề xuất đổi điểm, không phải tất cả cảnh đã được kiểm tra.

### Giao diện thử nghiệm — bước 1

Tại thư viện **Xưởng Game**, mở **Giao diện thử nghiệm** (riêng với **Xưởng Theme**). Mẫu Điện ảnh dùng hai cảnh và hai lời kết cố định trong bộ nhớ để thử bố cục điện thoại/máy tính, lựa chọn, phản hồi điểm, nhật ký và chữ lớn. Không đọc/ghi game, không gọi AI, không thêm/sửa theme hiện có, chưa tích hợp player thật hay xuất/embed. CSS chỉ nằm dưới `.cinema-lab`; minh họa tạm tái sử dụng ảnh cục bộ `hero-transmigration.jpg`. Đây là bước duyệt hướng thiết kế trước khi tích hợp dữ liệu thật.

Bước 2 của bản thử bổ sung các tình huống: truyện ngắn, truyện dài với tám đáp án, 12 chỉ số công khai, hoặc kết hợp cả hai. Có thể dán đoạn văn riêng vào cảnh đầu chỉ để xem bố cục; không phân tích thành game, không gửi AI hay lưu dữ liệu. Nội dung và đáp án cuộn tự nhiên, không cắt chữ; hình nền giới hạn chiều cao để không bị kéo giãn theo truyện. Bảng chỉ số cho ghim tối đa ba chỉ số lên thanh chính; số còn lại xem trong bảng chi tiết. Chỉ số bí mật trong dữ liệu mẫu không được hiện ở thanh, bảng hay phản hồi điểm. Popup mở theo cửa sổ trình duyệt để vẫn dùng được khi đọc ở cuối cảnh dài. Chưa kết nối dữ liệu hoặc luật của game thật.

Cập nhật bố cục thử: bỏ footer và thanh chỉ số thường trực. Một nút **Menu** ở góc trên mở toàn bộ chỉ số công khai, nhật ký lựa chọn và cài đặt đọc; không còn chọn ghim chỉ số. Thay đổi chỉ áp dụng cho khu thử nghiệm.

Menu bản thử có **Nhật ký hành trình**: lưu bản chụp toàn bộ đoạn văn, lời thoại, địa điểm/thời gian, đáp án đã chọn và phản hồi điểm công khai ở từng cảnh đã đi qua; gồm cả cảnh hiện tại/lời kết, không đưa nhánh chưa chơi vào nhật ký. Có thể mở/thu từng cảnh. **Chơi lại** và **Thoát game** có bước xác nhận trong menu; chơi lại đặt lại điểm/nhật ký, thoát trở về xưởng. Đây vẫn là tiến trình mẫu trong bộ nhớ, không tác động game đã lưu.

Bước 3 bổ sung **Bộ giao diện** với sáu mẫu độc lập: Điện ảnh · Tình cảm, Trang sách · Cổ tích, Hệ thống RPG, Bí ẩn · Điều tra, Cung đấu và Trọng sinh · Làm giàu. Mỗi mẫu có bố cục, chỉ số, hai cảnh thử và lời kết riêng; RPG có thông báo hệ thống, nhiệm vụ và kết thúc sớm khi sinh lực về 0. **Xem trước kết thúc** cho xem từng thiết kế mà không đổi điểm hoặc nhật ký của lượt đang chơi. Đổi thể loại bắt đầu lại mẫu. Header giữ nút Menu khi cuộn; các khung phụ xếp dọc trên điện thoại. Nội dung mẫu và ngưỡng kết thúc chỉ để thử, không áp vào game thật. Âm thanh dành cho bước tiếp theo, chưa triển khai.

### Viết lại toàn bộ một nhánh

Trong **Sơ Đồ Tư Duy → Xuất / nhập nhánh truyện**, hoặc ngay trên ô đáp án, mở **Viết lại toàn bộ một nhánh**. Xuất kịch bản Markdown theo cú pháp `CẢNH 1`, `A — lời đáp án`, `→ Đến cảnh 2`, `→ Kết thúc HE`, `KẾT THÚC HE [GOOD_END]`. Không giữ dấu đánh dấu từng đoạn hay mã cảnh cũ: được thêm/bớt cảnh, đổi điểm, điều kiện và bố cục. Cảnh đầu tiên trong bản dán là điểm vào mới; `→ Về cảnh nguồn` nối về cảnh trước nhánh.

Chỉ cần giữ **MÃ NHÁNH** ở đầu kịch bản. Lúc xuất, mã nhận diện được lưu trong `meta.branchRewriteTickets` của game qua cơ chế tự lưu hiện có (không cần thay schema). Khi nhập, mã xác định game/nhánh nguồn, không đoán theo tên NPC hoặc vị trí các cảnh trong văn bản. Đợi game tự lưu xong trước khi đóng trang. Cảnh, điểm hoặc đường vào nhánh thay đổi sau khi xuất thì bản cũ bị chặn; sửa lời văn ở nhánh độc lập khác không làm mất hiệu lực. Mã đã dùng không được nhập lần hai.

**Kiểm tra và nhận diện nhánh** hiển thị nhánh đích, số cảnh cũ riêng sẽ gỡ, số cảnh mới và cảnh chung được giữ. **Thay nhánh này bằng kịch bản mới** tạo tất cả cảnh với mã mới và chỉ đổi đích của đáp án mở nhánh; giữ thẻ NPC, điều kiện và hiệu ứng của đáp án nguồn. Cảnh dùng chung không bị xóa: bản viết lại dùng bản sao mới của chúng nếu tác giả vẫn giữ phần đó trong kịch bản. Nhánh khác và cấu hình toàn game không đổi. Tiến trình chơi nằm ở cảnh cũ bị gỡ cần chơi lại.
...
Giao diện mới có mục **Hiệu ứng nền** ngay dưới **Giao diện chơi**: tắt, tuyết, mưa, lá, cánh hoa, đom đóm, sao, sương. Lưu riêng trong `meta.readingEffect`; mặc định tắt và không thay hiệu ứng của theme cũ. Hiệu ứng CSS nhẹ, không chặn chuột, không kèm âm thanh; được nhúng trong HTML xuất và tự tắt khi thiết bị yêu cầu giảm chuyển động.

Bổ sung 4 giao diện theo thể loại, dựng bằng gradient CSS nhiều lớp (không cần ảnh riêng): **Học đường · Thanh xuân** (giấy kẻ ô vở, chấm hoa hồng phấn rải rác, khung bo tròn nét đứt), **Kinh dị · Ám ảnh** (nền gần đen, quầng tối viền khung, khung bo méo bất đối xứng), **Cyberpunk · Neon đô thị** (lưới neon lam/hồng, viền phát sáng, chữ monospace) và **U linh · Truyền kỳ** (quầng sáng đèn lồng vàng trên nền nâu đỏ thẫm, khung kết vòm như đèn lồng).

Bổ sung thêm 6 giao diện lấy đúng bảng màu từ các theme đọc truyện của LilyHub (`reader-bg-*` trong repo lilyhub) để đồng bộ nhận diện giữa trang đọc truyện và game: **Lily Mặc · Thuỷ mặc**, **Lily Mộng · Mộng hoa**, **Lily Tím · Yên tím**, **Lily Hồng · Giấy hồng**, **Lily Lam · Sương lam**, **Lily Mai · Bình minh**. Ba mẫu đầu có tranh minh hoạ SVG nội tuyến vẽ theo đúng kỹ thuật hoạ tiết của LilyHub — vài khối màu lớn làm mờ mạnh (`feGaussianBlur`) rải ở góc, vài nét cong mảnh, cụm chấm nhỏ mô phỏng cánh hoa/rêu, độ mờ dưới 30% để không bao giờ lấn chữ — không sao chép nguyên file SVG của LilyHub, chỉ dùng lại kỹ thuật và màu. Ba mẫu sau (Hồng, Lam, Mai) dùng đúng công thức gradient CSS của LilyHub, không cần ảnh. Khung chữ/lựa chọn của ba mẫu có tranh được làm trong suốt mạnh (khung ~40%, nút chọn ~34% độ đục) kèm hiệu ứng kính mờ để tranh hiện rõ xuyên suốt vùng đọc thay vì chỉ lộ viền ngoài — không phải ảnh chụp, không thêm file vào `public/`, vẫn tự chứa 100% trong HTML xuất. Cùng cơ chế `readingThemes.js` dùng chung cho GamePlayer và HTML xuất như các giao diện còn lại.
